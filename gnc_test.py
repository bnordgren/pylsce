...
class test_Add_Vars_to_Dict_by_RegSum():
    ...
    def test_grid_None(self):
        ...
        np.testing.assert_allclose(litcon1['LITTER_CONSUMP'],np.ma.sum(np.ma.sum(litter_consump,axis=-1),axis=-1))
        #testing mean; area_weight as None
        litcon1 = d.Add_Vars_to_Dict_by_RegSum(['LITTER_CONSUMP'],mode='mean')
        np.testing.assert_allclose(litcon1['LITTER_CONSUMP'],np.ma.sum(np.ma.sum(litter_consump,axis=-1),axis=-1)/gridcell_num)
        ...
        np.testing.assert_allclose(litcon1['LITTER_CONSUMP'],np.ma.sum(np.ma.sum(litter_consump_area,axis=-1),axis=-1)/total_area)
