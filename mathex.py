...
def plot_array_transg(pdata,interval_original,copy=False,interval_target=None):
    ...
    Note:
        ...
        3. it's possible to impose the interval you want the data to be transferred to by set interval_target. Note the target list length must be equal
           to length of pb.iteflat(interval_original). if interval_target == None, interval_target will be:
    ...
    Return:
        ...
        interval_target:in case of interval_target==None, return np.linspace(1,len(pb.iteflat(interval_original)),num=len(pb.iteflat(interval_original)),endpoint=True)
...
def OLS_RegGrid(prmf):
    """
    Purpose: Do OLS linear regression across a grid against the index (ie. sequential time axis)
    Note: 
        1. prmf should be a three dimension ndarray (eg. l X m X n), the regression is done for the grid mXn again time axis l
        2. the result is a 3XmXn array, with the first dimension in the sequence of slope, R2, pvalue.
...
def ncGet_OLS_Trend(infile,outfile,varlist='all',reginterval=None):
    ...
    for varindex,varname in enumerate(varlist2):
        ...
        if reg_start_end_index[varindex]==None:
...
def array_split(inarray,index,mode=None):
    """
...
def linreg_OLS_2var(x,y):
    ...
    Note:
        1. x,y can be masked array, or array with nan as missing data.
...
def ndarray_list_check_unique_shape(list_of_ndarray):
    ...
    """
    unique_dim = True
    shape0 = np.shape(list_of_ndarray[0])
    for array in list_of_ndarray[1:]:
        if np.shape(array) == shape0:
            pass
        else:
            unique_dim = False
            break
...
def ndarray_group_txt_by_func(filelist,outputfile=None,func=None,fmt='%.18e', delimiter=' ', newline='\n'):
    ...
    outputfile: if outputfile==None, the derived numpy ndarray will be returned, otherwise write the ndarray into output file.
